...
public class FileController {
    ...
    public ResponseEntity<Map<String,String>> uploadFile(
        ...
        if (fileName == null || fileName.trim().isEmpty()) {
            return ResponseEntity.badRequest()
                    .body(Map.of("error", "File name is required and cannot be empty"));
        }
        ...
        try {
            ...
            FileMetaData fileMetaData = new FileMetaData(fileID,
                    fileName.trim(), LocalDateTime.now(), file.getSize(), file.getContentType(), metaData, fileData);
    ...
    public ResponseEntity<?> updateFile(@PathVariable String fileID,
        ...
        try {
            FileMetaData fileMetaData = fileStorage.get(fileID.trim());
